...
\imagen{editor-style}{Definición de estilos para la invocación de procedimientos}
...
Para llevar esto a cabo se declaran acciones de transformación y reemplazo de nodos, para las cuales se declaran condiciones disparadoras. 
...
\imagen{aritmetic-operator}{Edición de operaciones binarias sin acciones de reemplazo}
...
De manera automática y sin que el usuario lo note, mostrándole \fig{aritmetic-operator-action}

\imagen{aritmetic-operator-action}{Edición de operaciones binarias con acciones de reemplazo definidas}
...
Al escribir el \emph{if} se autogenera la plantilla correspondiente \fig{if-statement}

\imagen{if-statement}{Editor de la alternativa condicional con dos ramas}
...
Siguiendo la filosofía planteada en este trabajo, la definición del ejercicio también se visualiza como un archivo, orientado a ser legible y con apariencia de documentación \fig{ejercicio1}

\imagen{ejercicio1}{Definición de un ejercicio simple}

A su vez, dentro de la definición del ejercicio es posible declarar qué herramientas del lenguaje no pueden ser usadas dentro de la resolución del mismo \fig{restricciones-ejercicio}
...
La capacidad de definir ejercicios con sus respectivas especificaciones, tableros, bibliotecas y restricciones constituye un aporte novedoso respecto de las implementaciones pre-existentes. Por este mismo motivo la utilidad de esta funcionalidad, así como también posibles modificaciones futuras, queda aún por ser demostrada y dependerá de la respuesta de los usuarios. Otras funcionalidades, no especificadas en \citetitle{Gobstones} pero presentes en \textit{PyGobstones}, como la posibilidad de cambiar vestimentas (estilos e imágenes) a los tableros, han resultado ser sumamente útiles a la secuencia didáctica de Gobstones y se proyecta agregarlas en versiones futuras del producto.
